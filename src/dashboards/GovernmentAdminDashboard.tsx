        ...
        {nav.map(([id, label, Icon], idx) => {
          ...
          return (
            <button
              ...
              className={`w-full text-left p-3 rounded-xl flex gap-3 items-center text-sm font-medium transition-all ${
                active
  ...
  return (
    <div className="min-h-screen bg-app text-ink flex overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden min-w-0">
        ...
        {/* Content area */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 custom-scrollbar animate-in fade-in duration-200">
...
function Stat({ n, t, icon: Icon, go }: { n: number; t: string; icon: typeof Building2; go: () => void }) {
  return (
    <button
      onClick={go}
      className="bg-surface border border-line rounded-2xl p-5 text-left hover:-translate-y-1 hover:shadow-md transition-all group"
    >
