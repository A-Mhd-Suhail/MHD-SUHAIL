...
const GovernmentAdminDashboard = lazy(() => import('./GovernmentAdminDashboard'));
...
export default function App() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (portal === 'doctor' && (!f.name || !f.specialization || !f.regNo || !f.hospital)) { setAuthError('Doctor name, specialization, registration number, and hospital name are required.'); return; }
    if ((portal === 'hospital' || portal === 'admin') && !f.name) { setAuthError('Hospital name is required.'); return; }
    setAuthLoading(true);
    try {
      ...
      try {
        ...
        if (portal === 'patient') {
          ...
          Object.assign(profile, {
            ...
            hospital: f.hospital, regNo: f.regNo, phone: f.phone || '', onDuty: false,
          });
        } else {
          Object.assign(profile, {
  ...
  if (loggedIn) {
    const Dashboard =
      loggedIn === 'patient' ? PatientDashboard : loggedIn === 'doctor' ? DoctorDashboard : loggedIn === 'hospital' ? AdminDashboard : GovernmentAdminDashboard;
    return (
