  ...
  return (
    <div className="max-w-[1000px] mx-auto space-y-6 pb-12">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button onClick={() => go(t('appts'))} className="h-[40px] px-4 bg-surface border border-primary text-primary rounded-[10px] text-[13px] font-semibold hover:bg-active transition-colors">
          {t('bookAppt')}
        </button>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <div className="bg-surface text-ink border border-line border-l-4 border-l-primary rounded-2xl p-5 shadow-sm">
          <p className="text-[11px] font-bold text-muted uppercase tracking-wider mb-3">{t('nextApptLbl')}</p>
          {nextAppt ? (
            <>
              <p className="text-[15px] font-semibold text-heading">{nextAppt.doctorName}</p>
              <p className="text-[13px] text-muted mt-1">{fmtD(nextAppt.date)} · {nextAppt.time} · Queue #{queueNumberOf(appts, nextAppt)}</p>
              <p className="text-[12px] text-muted mt-1">{nextAppt.type}{nextAppt.hospital ? ` · ${nextAppt.hospital}` : ''}</p>
              <button onClick={() => go(t('appts'))} className="mt-3 text-[13px] font-semibold text-primary hover:underline">{t('viewDetails')}</button>
            </>
          ) : (
            <>
              <p className="text-[13px] text-muted">{t('noUpcomingAppts')}</p>
              <button onClick={() => go(t('appts'))} className="mt-3 text-[13px] font-semibold text-primary hover:underline">{t('bookAppt')} →</button>
            </>
          )}
          {dueFollowups.length > 0 && (
            <div className="mt-3 text-[12px] text-warn bg-warn-bg border border-warn-bd rounded-lg p-2.5">
