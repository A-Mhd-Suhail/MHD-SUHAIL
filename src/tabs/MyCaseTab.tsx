      ...
      <div>
        ...
        {list.length === 0 ? (
          ...
          <div className="space-y-3">
            ...
            {list.length > 3 && <button onClick={() => setShowAllCases((v) => !v)} className="text-[13px] font-medium text-primary hover:underline">{showAllCases ? 'Show fewer cases' : `View all ${list.length} cases`}</button>}
          </div>
        )}
      </div>
